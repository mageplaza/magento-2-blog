    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '1.1.1', '<')) {
            ...
            if (!$installer->tableExists('mageplaza_blog_post_traffic')) {
                $table = $installer->getConnection()
                    ...
                    ->addForeignKey(
                        $installer->getFkName('mageplaza_blog_post_traffic', 'post_id', 'mageplaza_blog_post',
                            'post_id'),
                        'post_id',
                    ...
                    ->addIndex(
                        $installer->getIdxName('mageplaza_blog_post_traffic', ['post_id', 'traffic_id'],
            ...
            if (!$installer->tableExists('mageplaza_blog_author')) {
                $table = $installer->getConnection()
                    ...
                    ->addIndex(
                        $installer->getIdxName('mageplaza_blog_author', ['user_id'],
                            AdapterInterface::INDEX_TYPE_UNIQUE),
                        ['user_id'],
                        ['type' => AdapterInterface::INDEX_TYPE_UNIQUE]
                    )
        ...
        if (version_compare($context->getVersion(), '1.1.2', '<')) {
            if ($installer->tableExists('mageplaza_blog_post')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_post'), 'meta_robots',
                    ['type' => Table::TYPE_TEXT]);
            }
            if ($installer->tableExists('mageplaza_blog_tag')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_tag'), 'meta_robots',
                    ['type' => Table::TYPE_TEXT]);
            }
            if ($installer->tableExists('mageplaza_blog_category')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_category'), 'meta_robots',
                    ['type' => Table::TYPE_TEXT]);
            }
            if ($installer->tableExists('mageplaza_blog_topic')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_topic'), 'meta_robots',
                    ['type' => Table::TYPE_TEXT]);
            }
            ...
            if (!$installer->tableExists('mageplaza_blog_comment')) {
                $table = $installer->getConnection()
                    ...
                    ->addColumn('post_id', Table::TYPE_INTEGER, null, ['unsigned' => true, 'nullable' => false,],
                        'Post ID')
                    ->addColumn('entity_id', Table::TYPE_INTEGER, null, ['unsigned' => true, 'nullable' => false,],
                        'User Comment ID')
                    ->addColumn('has_reply', Table::TYPE_SMALLINT, 2,
                        ['unsigned' => true, 'nullable' => false, 'default' => 0], 'Comment has reply')
                    ->addColumn('is_reply', Table::TYPE_SMALLINT, 2,
                        ['unsigned' => true, 'nullable' => false, 'default' => 0], 'Is reply comment')
                    ->addColumn('reply_id', Table::TYPE_INTEGER, null,
                        ['unsigned' => true, 'nullable' => true, 'default' => 0], 'Reply ID')
                    ->addColumn('content', Table::TYPE_TEXT, 255, [], 'Comment content')
                    ->addColumn('created_at', Table::TYPE_TEXT, null, [], 'Comment Created At')
                    ->addColumn('status', Table::TYPE_SMALLINT, 3,
                        ['unsigned' => true, 'nullable' => false, 'default' => 3], 'Status')
            ...
            if (!$installer->tableExists('mageplaza_blog_comment_like')) {
                $table = $installer->getConnection()
                    ...
                    ->addColumn('comment_id', Table::TYPE_INTEGER, null, ['unsigned' => true, 'nullable' => false,],
                        'Comment ID')
                    ->addColumn('entity_id', Table::TYPE_INTEGER, null, ['unsigned' => true, 'nullable' => false,],
                        'User Like ID')
                    ->addIndex($installer->getIdxName('mageplaza_blog_comment_like', ['like_id']), ['like_id'])
                    ->addForeignKey(
                        $installer->getFkName('mageplaza_blog_comment_like', 'comment_id', 'mageplaza_blog_comment',
                            'comment_id'),
                        'comment_id',
                        ...
                        $installer->getFkName('mageplaza_blog_comment_like', 'entity_id', 'customer_entity',
                            'entity_id'),
                        'entity_id',
                        $installer->getTable('customer_entity'),
                        'entity_id',
        ...
        if (version_compare($context->getVersion(), '1.1.4', '<')) {
            if (!$installer->tableExists('mageplaza_blog_post_product')) {
                $table = $installer->getConnection()
                    ...
                    ->addForeignKey(
                        $installer->getFkName('mageplaza_blog_post_product', 'post_id', 'mageplaza_blog_post',
                            'post_id'),
                        'post_id',
                        $installer->getTable('mageplaza_blog_post'),
                        'post_id',
                    ...
                    ->addForeignKey(
                        $installer->getFkName('mageplaza_blog_post_product', 'entity_id', 'catalog_product_entity',
                            'entity_id'),
                        'entity_id',
                    ...
                    ->addIndex(
                        $installer->getIdxName('mageplaza_blog_post_product', ['post_id', 'entity_id'],
                            AdapterInterface::INDEX_TYPE_UNIQUE),
                        ['post_id', 'entity_id'],
                        ['type' => AdapterInterface::INDEX_TYPE_UNIQUE]
                    )
        ...
        if (version_compare($context->getVersion(), '2.4.3', '<')) {
            if ($installer->tableExists('mageplaza_blog_post')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_post'), 'created_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_post')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_post'), 'updated_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_post')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_post'), 'publish_date',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_tag')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_tag'), 'created_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_tag')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_tag'), 'updated_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_category')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_category'), 'created_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_category')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_category'), 'updated_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_topic')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_topic'), 'created_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
            if ($installer->tableExists('mageplaza_blog_topic')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_topic'), 'updated_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
        }
        ...
        if (version_compare($context->getVersion(), '2.4.4', '<')) {
            if ($installer->tableExists('mageplaza_blog_comment')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_comment'), 'content',
                    ['type' => Table::TYPE_TEXT]);
            }
            if ($installer->tableExists('mageplaza_blog_comment')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_comment'), 'created_at',
                    ['type' => Table::TYPE_DATETIME]);
            }
        ...
        if (version_compare($context->getVersion(), '2.4.5', '<')) {
            if ($installer->tableExists('mageplaza_blog_post_traffic')) {
                $connection->modifyColumn($installer->getTable('mageplaza_blog_post_traffic'), 'numbers_view',
                    ['type' => Table::TYPE_INTEGER]);
            }
        }
